import os
import re
import secrets

import numpy as np
import folder_paths
import wave
from scipy.io.wavfile import write as write_wav


# Based on "Save Audio With Path (CRT)" by plugcrypt (crt-nodes), MIT License.
# Extended with ComfyUI-style sequential file numbering (_001, _002, ...).

CLIP_CEILING_DBFS = -1.0
CLIP_CEILING = 10 ** (CLIP_CEILING_DBFS / 20.0)


def _write_pcm24_wav(path, sample_rate, audio_data):
    if audio_data.ndim == 1:
        audio_data = audio_data[:, None]

    audio_i24 = np.round(audio_data * 8388607.0).astype("<i4")
    audio_i24 = np.clip(audio_i24, -8388608, 8388607)
    packed = audio_i24.reshape(-1, 1).view(np.uint8).reshape(-1, 4)[:, :3]

    with wave.open(path, "wb") as wav:
        wav.setnchannels(audio_data.shape[1])
        wav.setsampwidth(3)
        wav.setframerate(int(sample_rate))
        wav.writeframes(packed.tobytes())


def _next_counter(final_dir, stem):
    """Next free sequential number for '<stem>_<N>.wav' inside final_dir.

    Same rule as ComfyUI's core Save Image node: find the highest existing index
    of files named <stem>_<digits>.wav (any digit padding) and return it + 1.
    A trailing existence check guarantees an existing file is never overwritten,
    even if it does not follow the naming scheme.
    """
    counter = 1
    pattern = re.compile(r"^" + re.escape(stem) + r"_(\d+)\.wav$")
    try:
        for name in os.listdir(final_dir):
            m = pattern.match(name)
            if m:
                counter = max(counter, int(m.group(1)) + 1)
    except FileNotFoundError:
        pass

    while os.path.exists(os.path.join(final_dir, f"{stem}_{counter}.wav")):
        counter += 1
    return counter


class SaveAudioWithPathNumbered:
    OUTPUT_NODE = True

    @classmethod
    def INPUT_TYPES(cls):
        output_dir = folder_paths.get_output_directory()
        return {
            "required": {
                "audio": ("AUDIO",),
                "folder_path": (
                    "STRING",
                    {
                        "default": output_dir,
                        "tooltip": "Base folder (or a full file path to get the folder from). Defaults to ComfyUI's output folder.",
                    },
                ),
                "subfolder_name": (
                    "STRING",
                    {"default": "audio", "tooltip": "Subfolder to create within the base folder. Leave empty to save directly into the base folder."},
                ),
                "filename": (
                    "STRING",
                    {
                        "default": "output",
                        "tooltip": "File name for the audio file (without extension). The sequential number is appended automatically when numbering is enabled.",
                    },
                ),
                "suffix": ("STRING", {"default": "", "tooltip": "Optional suffix to append to the filename (before the number), e.g. '_v2'."}),
                "numbering": (
                    ["zero-padded", "plain", "off"],
                    {
                        "default": "zero-padded",
                        "tooltip": "'zero-padded' -> output_001.wav, _002.wav ... ; 'plain' -> output_1.wav, _2.wav ... ; 'off' -> legacy CRT behaviour (overwrites).",
                    },
                ),
                "digits": (
                    "INT",
                    {
                        "default": 3,
                        "min": 1,
                        "max": 9,
                        "tooltip": "Number of digits for zero-padded numbering (_001 vs _1). Ignored in 'plain' mode.",
                    },
                ),
                "sample_rate": (
                    ["44100", "48000", "88200", "96000", "192000"],
                    {
                        "default": "44100",
                        "tooltip": "Fallback audio sample rate in Hz if not in audio data.",
                    },
                ),
                "bit_depth": (
                    ["24-bit PCM", "32-bit float"],
                    {
                        "default": "24-bit PCM",
                        "tooltip": "WAV encoding. 24-bit PCM is the default; 32-bit float can preserve over-full-scale samples when normalization is disabled.",
                    },
                ),
                "normalize_clipping": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "If enabled, peak-normalizes clipped audio. If disabled, 24-bit PCM hard-clips to -1 dBFS while 32-bit float saves over-full-scale samples.",
                    },
                ),
            }
        }

    RETURN_TYPES = ()
    FUNCTION = "save_audio"
    CATEGORY = "CRT/Save"
    DESCRIPTION = (
        "Saves audio as an uncompressed WAV file to a specified folder with automatic "
        "sequential numbering (_001, _002, ...) in the style of ComfyUI's Save Image node."
    )

    def save_audio(
        self,
        audio,
        folder_path,
        subfolder_name,
        filename,
        suffix="",
        numbering="zero-padded",
        digits=3,
        sample_rate="44100",
        bit_depth="24-bit PCM",
        normalize_clipping=True,
    ):
        if audio is None:
            print("[ERROR] ERROR: No input audio provided to SaveAudioWithPathNumbered.")
            return ()

        try:
            # If folder_path actually points at a file, use its parent directory.
            base_path = os.path.dirname(folder_path) if os.path.isfile(folder_path) else folder_path
            if os.path.isfile(folder_path):
                print(f"[INFO] Input 'folder_path' was a file. Using its parent directory: {base_path}")

            subfolder_clean = subfolder_name.strip().lstrip("/\\")
            filename_clean = filename.strip().lstrip("/\\")
            suffix_clean = suffix.strip()

            # Empty filename: auto-generate a unique name (never block the save).
            if not filename_clean:
                filename_clean = f"audio_{secrets.token_hex(16)}"
                print(f"[INFO] No filename given. Using auto-generated name: {filename_clean}")

            stem = f"{filename_clean}{suffix_clean}"

            final_dir = os.path.join(base_path, subfolder_clean) if subfolder_clean else base_path
            os.makedirs(final_dir, exist_ok=True)

            # --- File naming / numbering (Save Image style) ---
            digits = int(max(1, min(9, digits)))
            if numbering == "zero-padded":
                counter = _next_counter(final_dir, stem)
                final_filepath = os.path.join(final_dir, f"{stem}_{counter:0{digits}d}.wav")
            elif numbering == "plain":
                counter = _next_counter(final_dir, stem)
                final_filepath = os.path.join(final_dir, f"{stem}_{counter}.wav")
            else:  # "off" — legacy CRT behaviour
                final_filepath = os.path.join(final_dir, f"{stem}.wav")
                if os.path.exists(final_filepath):
                    print("[WARN] File already exists and numbering is off - it will be overwritten.")

            # --- Audio data handling (same as original CRT node) ---
            if not isinstance(audio, dict) or "waveform" not in audio:
                raise TypeError(
                    "Input 'audio' is not in the expected format. It must be a dictionary with a 'waveform' key."
                )

            waveform_tensor = audio["waveform"]
            sr_to_use = int(audio.get("sample_rate", sample_rate))

            if waveform_tensor is None or waveform_tensor.nelement() == 0:
                print("[ERROR] ERROR: The 'waveform' tensor in the audio input is empty.")
                return ()

            audio_data = waveform_tensor[0].cpu().numpy()
            audio_data = audio_data.T

            peak = np.max(np.abs(audio_data))
            if peak > 1.0:
                if normalize_clipping:
                    print("[WARN] Warning: Audio data is clipping. It will be normalized.")
                    audio_data /= peak
                elif bit_depth == "32-bit float":
                    print("[WARN] Warning: Audio data is clipping. Saving over-full-scale samples as 32-bit float.")
                else:
                    print("[WARN] Warning: Audio data is clipping. Hard-clipping peaks to -1 dBFS.")
                    audio_data = np.clip(audio_data, -CLIP_CEILING, CLIP_CEILING)

            if bit_depth == "32-bit float":
                write_wav(final_filepath, sr_to_use, audio_data.astype(np.float32))
            else:
                audio_data = np.clip(audio_data, -1.0, 1.0)
                _write_pcm24_wav(final_filepath, sr_to_use, audio_data)

            print(f"[OK] Saved audio successfully to: {final_filepath}")
            return ()

        except Exception as e:
            print(f"[ERROR] ERROR in SaveAudioWithPathNumbered: {str(e)}")
            raise e


# Dictionary mappings for ComfyUI
NODE_CLASS_MAPPINGS = {"SaveAudioWithPathNumbered": SaveAudioWithPathNumbered}
NODE_DISPLAY_NAME_MAPPINGS = {"SaveAudioWithPathNumbered": "Save Audio With Path (Numbered)"}
